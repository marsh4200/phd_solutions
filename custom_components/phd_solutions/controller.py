...
class PHDController:
    ...
    async def async_test_connection(self, timeout: float = 2.0):
        """Try to connect to host:port. Raise if it fails."""
        try:
            ...
            if self.debug:
                _LOGGER.debug(
                    "Connection test succeeded to %s:%s",
                    self.host,
                    self.port,
                )
            return True
            ...
            _LOGGER.warning(
                "Connection test failed to %s:%s — %s",
                self.host,
                self.port,
                err,
            )
    ...
    async def send_command(self, cmd: str, timeout: float = 2.0):
        ...
        try:
            ...
            _LOGGER.error("Send failed to %s:%s -> %s", self.host, self.port, err)
            raise